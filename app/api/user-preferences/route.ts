...
export async function POST(request: NextRequest){
    ...
    if(!categories || !Array.isArray(categories) || categories.length === 0){
        return NextResponse.json(
            {error: "Categories array is required and myst not be empty"},
            {status: 400}
        );
    }
    ...
    if(!frequency || !["daily","weekly","biweekly"].includes(frequency)){
        return NextResponse.json(
            {error:"valid frquency is required (daily, weekly, biweekly)"},
            {status: 400}
        );
    }
    ...
    return NextResponse.json({
        success:true,
        message:"preparation saved and added to table",
    });
}
...
export async function PATCH(request: NextRequest){
    ...
    try {
        ...
        if(updateError) {
            return NextResponse.json(
                {error: " failed tp get preferences"},
                {status: 500}
            );
        }
